...
main! = |_args| {
	...
	Assert.eq(Playwright.text_content!(page, TodoPage.label(1))?, "padded") ? |e| TitleShouldBeTrimmed(e)

	# Whitespace only trims down to nothing, so nothing is added
	TodoPage.add!(page, "   ")?
	Assert.eq(Playwright.query_count!(page, ".todo-list li")?, 1) ? |e| BlankTitleShouldAddNothing(e)
